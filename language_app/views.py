...
def home_view(request):
    if request.method == "POST":
        user = request.user
        response_data = {}
        if request.POST.get("status") == "quiz":
            response_data["redirect_url"] = reverse("quiz")
            return HttpResponse(json.dumps(response_data), content_type="application/json")
        if request.POST.get("status") == "add_word":
            response_data["redirect_url"] = reverse("add_word")
            return HttpResponse(json.dumps(response_data), content_type="application/json")
        if request.POST.get("status") == "get_analise_report":
        ...
        if request.POST.get("status") == "settings":
            response_data["redirect_url"] = reverse("settings")
            return HttpResponse(json.dumps(response_data), content_type="application/json")
    else:
...
def quiz_view(request):
    if request.method == "POST":
        user = request.user
        if request.POST.get("status") == "get_quiz_data":
            words = create_quiz_data(user)
            return HttpResponse(json.dumps(words), content_type="application/json")
        if request.POST.get("status") == "send_quiz_data":
            correct_answers = []
            wrong_answers = []
